...
function validateBinaryVersion(...command) {
  ...
  if (stdout !== "0.13.5") {
    throw new Error(`Expected ${JSON.stringify("0.13.5")} but got ${JSON.stringify(stdout)}`);
  }
}
function isYarn() {
...
function installUsingNPM(pkg, subpath, binPath) {
  ...
  try {
    ...
    child_process.execSync(`npm install --loglevel=error --prefer-offline --no-audit --progress=false ${pkg}@${"0.13.5"}`, { cwd: installDir, stdio: "pipe", env });
...
async function downloadDirectlyFromNPM(pkg, subpath, binPath) {
  const url = `https://registry.npmjs.org/${pkg}/-/${pkg}-${"0.13.5"}.tgz`;
