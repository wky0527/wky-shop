const Mock = require('mockjs');
const express = require('express');
const router = express.Router();
router.use("/home/all", (req,res)=>{
    const data = Mock.mock({
        swiper: ['http://www.ftsucai.com/demo/202102/appmb010/themes/img/banner/news-banner1.jpg',
                'http://www.ftsucai.com/demo/202102/appmb010/themes/img/banner/news-banner2.jpg',
                'http://www.ftsucai.com/demo/202102/appmb010/themes/img/banner/news-banner3.jpg',
                'http://www.ftsucai.com/demo/202102/appmb010/themes/img/banner/news-banner1.jpg'
                ],
        data: [],
        entry: [
            {
                title: '每日签到',
                icon: 'icon-qiandao',
                 bgColor: 'rgb(88,180,240)'
            },
            {
                title: '限时抢购',
                icon: 'icon-qiandao',
                bgColor: 'rgb(226,97,92)'

            },
            {
                title: '分享领券',
                icon: 'icon-qiandao',
                bgColor: 'rgb(88,180,240)'

            },
            {
                title: '会员专享',
                icon: 'icon-qiandao',
                bgColor: 'rgb(226,97,92)'

            },
            {
                title: '每日签到',
                icon: 'icon-qiandao',
                bgColor: 'rgb(88,180,240)'

            },
            {
                title: '限时抢购',
                icon: 'icon-qiandao',
                bgColor: 'rgb(226,97,92)'

            },
            {
                title: '分享领券',
                icon: 'icon-qiandao',
                bgColor: 'rgb(88,180,240)'

            },
            {
                title: '会员专享',
                icon: 'icon-qiandao',
                bgColor: 'rgb(226,97,92)'
            }
        ],
        list: [
            {
                title: '电器城抢购',
                description: '全场低至299',
                img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/f1.jpg'
            },
            {
                title: '创维新品',
                description: '好品质选创维新品上市',
                img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/f2.jpg'
            },
            {
                title: '拍摄一族',
                description: '喜欢旅游拍摄',
                img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/f3.jpg'
            },
            {
                title: '每日坚果',
                description: '每天补充一下',
                img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/f5.jpg'
            },
            {
                title: '',
                description: '',
                img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/pd/cf-1.jpg'
            },
            {
                title: '',
                description: '',
                img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/pd/cf-3.jpg'
            }
        ],
        category: [
            {
                id:1,
                title: '热门推荐',
                img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/pd/cf-4.jpg',
                data: [
                    {
                        name: '手机数码',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-1.jpg'
                    },
                    {
                        name: '笔记本',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-2.jpg'
                    },
                    {
                        name: '空调',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-3.jpg'
                    },
                    {
                        name: '儿童文学',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-4.jpg'
                    },
                    {
                        name: '啤酒',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-5.jpg'
                    },
                    {
                        name: '海参',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-6.jpg'
                    },
                    {
                        name: '床上',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-7.jpg'
                    },
                    {
                        name: '衣物清洁',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-8.jpg'
                    },
                    {
                        name: '羽绒服',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-9.jpg'
                    }
                ]
            },
            {
                id:2,
                title: '美容彩妆',
                img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/pd/cf-4.jpg',
                data: [
                    {
                        name: '手机数码',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-1.jpg'
                    },
                    {
                        name: '笔记本',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-2.jpg'
                    },
                    {
                        name: '空调',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-3.jpg'
                    },
                    {
                        name: '儿童文学',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-4.jpg'
                    },
                    {
                        name: '啤酒',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-5.jpg'
                    },
                    {
                        name: '海参',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-6.jpg'
                    },
                    {
                        name: '床上',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-7.jpg'
                    },
                    {
                        name: '衣物清洁',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-8.jpg'
                    },
                    {
                        name: '羽绒服',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-9.jpg'
                    }
                ]
            },
            {
                id:3,
                title: '母婴专区',
                img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/pd/cf-4.jpg',
                data: [
                    {
                        name: '手机数码',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-1.jpg'
                    },
                    {
                        name: '笔记本',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-2.jpg'
                    },
                    {
                        name: '空调',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-3.jpg'
                    },
                    {
                        name: '儿童文学',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-4.jpg'
                    },
                    {
                        name: '啤酒',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-5.jpg'
                    },
                    {
                        name: '海参',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-6.jpg'
                    },
                    {
                        name: '床上',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-7.jpg'
                    },
                    {
                        name: '衣物清洁',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-8.jpg'
                    },
                    {
                        name: '羽绒服',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-9.jpg'
                    }
                ]
            },
            {
                id:4,
                title: '箱包配饰',
                img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/pd/cf-4.jpg',
                data: [
                    {
                        name: '手机数码',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-1.jpg'
                    },
                    {
                        name: '笔记本',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-2.jpg'
                    },
                    {
                        name: '空调',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-3.jpg'
                    },
                    {
                        name: '儿童文学',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-4.jpg'
                    },
                    {
                        name: '啤酒',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-5.jpg'
                    },
                    {
                        name: '海参',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-6.jpg'
                    },
                    {
                        name: '床上',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-7.jpg'
                    },
                    {
                        name: '衣物清洁',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-8.jpg'
                    },
                    {
                        name: '羽绒服',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-9.jpg'
                    }
                ]
            },
            {
                id:5,
                title: '家居个护',
                img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/pd/cf-4.jpg',
                data: [
                    {
                        name: '手机数码',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-1.jpg'
                    },
                    {
                        name: '笔记本',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-2.jpg'
                    },
                    {
                        name: '空调',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-3.jpg'
                    },
                    {
                        name: '儿童文学',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-4.jpg'
                    },
                    {
                        name: '啤酒',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-5.jpg'
                    },
                    {
                        name: '海参',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-6.jpg'
                    },
                    {
                        name: '床上',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-7.jpg'
                    },
                    {
                        name: '衣物清洁',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-8.jpg'
                    },
                    {
                        name: '羽绒服',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-9.jpg'
                    }
                ]
            },
            {
                id:6,
                title: '营养保健',
                img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/pd/cf-4.jpg',
                data: [
                    {
                        name: '手机数码',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-1.jpg'
                    },
                    {
                        name: '笔记本',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-2.jpg'
                    },
                    {
                        name: '空调',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-3.jpg'
                    },
                    {
                        name: '儿童文学',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-4.jpg'
                    },
                    {
                        name: '啤酒',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-5.jpg'
                    },
                    {
                        name: '海参',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-6.jpg'
                    },
                    {
                        name: '床上',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-7.jpg'
                    },
                    {
                        name: '衣物清洁',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-8.jpg'
                    },
                    {
                        name: '羽绒服',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-9.jpg'
                    }
                ]
            },
            {
                id:7,
                title: '服饰鞋靴',
                img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/pd/cf-4.jpg',
                data: [
                    {
                        name: '手机数码',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-1.jpg'
                    },
                    {
                        name: '笔记本',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-2.jpg'
                    },
                    {
                        name: '空调',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-3.jpg'
                    },
                    {
                        name: '儿童文学',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-4.jpg'
                    },
                    {
                        name: '啤酒',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-5.jpg'
                    },
                    {
                        name: '海参',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-6.jpg'
                    },
                    {
                        name: '床上',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-7.jpg'
                    },
                    {
                        name: '衣物清洁',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-8.jpg'
                    },
                    {
                        name: '羽绒服',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-9.jpg'
                    }
                ]
            },
            {
                id:8,
                title: '数码家电',
                img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/pd/cf-4.jpg',
                data: [
                    {
                        name: '手机数码',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-1.jpg'
                    },
                    {
                        name: '笔记本',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-2.jpg'
                    },
                    {
                        name: '空调',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-3.jpg'
                    },
                    {
                        name: '儿童文学',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-4.jpg'
                    },
                    {
                        name: '啤酒',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-5.jpg'
                    },
                    {
                        name: '海参',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-6.jpg'
                    },
                    {
                        name: '床上',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-7.jpg'
                    },
                    {
                        name: '衣物清洁',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-8.jpg'
                    },
                    {
                        name: '羽绒服',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-9.jpg'
                    }
                ]
            },
            {
                id:9,
                title: '运动户外',
                img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/pd/cf-4.jpg',
                data: [
                    {
                        name: '手机数码',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-1.jpg'
                    },
                    {
                        name: '笔记本',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-2.jpg'
                    },
                    {
                        name: '空调',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-3.jpg'
                    },
                    {
                        name: '儿童文学',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-4.jpg'
                    },
                    {
                        name: '啤酒',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-5.jpg'
                    },
                    {
                        name: '海参',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-6.jpg'
                    },
                    {
                        name: '床上',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-7.jpg'
                    },
                    {
                        name: '衣物清洁',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-8.jpg'
                    },
                    {
                        name: '羽绒服',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-9.jpg'
                    }
                ]
            },
            {
                id:10,
                title: '汽车用品',
                img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/pd/cf-4.jpg',
                data: [
                    {
                        name: '手机数码',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-1.jpg'
                    },
                    {
                        name: '笔记本',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-2.jpg'
                    },
                    {
                        name: '空调',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-3.jpg'
                    },
                    {
                        name: '儿童文学',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-4.jpg'
                    },
                    {
                        name: '啤酒',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-5.jpg'
                    },
                    {
                        name: '海参',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-6.jpg'
                    },
                    {
                        name: '床上',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-7.jpg'
                    },
                    {
                        name: '衣物清洁',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-8.jpg'
                    },
                    {
                        name: '羽绒服',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-9.jpg'
                    }
                ]
            },
            {
                id:11,
                title: '生活旅行',
                img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/pd/cf-4.jpg',
                data: [
                    {
                        name: '手机数码',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-1.jpg'
                    },
                    {
                        name: '笔记本',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-2.jpg'
                    },
                    {
                        name: '空调',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-3.jpg'
                    },
                    {
                        name: '儿童文学',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-4.jpg'
                    },
                    {
                        name: '啤酒',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-5.jpg'
                    },
                    {
                        name: '海参',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-5.jpg'
                    },
                    {
                        name: '床上',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-5.jpg'
                    },
                    {
                        name: '衣物清洁',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-5.jpg'
                    },
                    {
                        name: '羽绒服',
                        img: 'http://m.ftsucai.com/demo/202102/appmb010/themes/img/ad/x-sf-9.jpg'
                    }
                ]
            }
        ]

    })
    return res.json(data)
})
router.use("/home/navigation",(req,res)=>{
    const data = Mock.mock({
        data: [
            {
                id: 1,
                name: '首页',
                path: '/home',
                exact: true,
                icon: 'f01.png',
                activeIcon: 'f1.png',
            },
            {
                id:2,
                name: '分类',
                path: '/category',
                exact: false,
                icon: 'f05.png',
                activeIcon: 'f5.png'
            },
            {
                id:3,
                name: '发现',
                path: '/found',
                exact: false,
                icon: 'f02.png',
                activeIcon: 'f2.png'
            },
            {
                id:4,
                name: '购物车',
                path: 'cart',
                exact: false,
                icon: 'f03.png',
                activeIcon: 'f3.png'
            },
            {
                id:5,
                name: '我的',
                path: 'my',
                exact: false,
                icon: 'f04.png',
                activeIcon: 'f4.png'
            }
        ]
    })
    return res.json(data)
})
module.exports = router;
// import {homeAllData,navigationData} from "./index.ts";
// export default [
//     {
//         url: '/backend/home/all',
//         method: 'get',
//         response: () => {
//             return {
//                 code: 200,
//                 message: 'ok',
//                 data: homeAllData
//             }
//         }
//     },
//     {
//         url: '/backend/home/navigation',
//         method: 'get',
//         response: () => {
//             return {
//                 code: 200,
//                 message: 'ok',
//                 data: navigationData
//             }
//         }
//     }
// ]
